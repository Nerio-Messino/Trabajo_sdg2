...
static port_button_hw_t buttons_arr[] = {
    ...
    /*Sigo sin entender porque no me reconoce el BUTTON_0_ID*/
};
void port_button_init(uint32_t button_id)
{
    ...
    /* TO-DO alumnos */
     port_system_gpio_config(p_port,pin,);
     port_system_gpio_config_exti(p_port,pin,button_id);
     port_system_gpio_exti_enable(pin,1,0);
     /*No entiendo que valores hay que dar muy bien */
}
...
void EXTI15_10_IRQHandler(void)
{
    ...
    if (EXTI->PR & BIT_POS_TO_MASK(buttons_arr[BUTTON_0_ID].pin))/*No reconoce BIT_POS_TO_MASK*/
    {
      port_button_read(buttons_arr);
      if(check_button_released()){
        port_button_hw_t flag_pressed -> false;/*Esto es un booleano , porque no me deja darle un valor */
      }else{
        port_button_hw_t flag_pressed = true;
      }
